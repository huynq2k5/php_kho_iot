        ...
        <ul class="flex items-center flex-shrink-0 space-x-6">
            ...
            <li class="relative">
                <button class="relative align-middle rounded-md focus:outline-none focus:shadow-outline-red" 
                    ...
                    <span id="main-unread-badge" class="absolute top-0 right-0 inline-flex items-center justify-center w-4 h-4 text-[10px] font-bold text-white transform translate-x-1 -translate-y-1 bg-red-600 border-2 border-white rounded-full dark:border-gray-800 hidden">
                        1
                    </span>
                </button>
                ...
                <template x-if="isNotificationsMenuOpen">
                    <ul x-transition:leave="transition ease-in duration-150" 
                        ...
                        aria-label="submenu">
                        <li class="flex">
                            <a class="inline-flex items-center justify-between w-full px-2 py-1 text-sm font-semibold ..." href="index.php?page=alert_log">
                                <span>Chưa đọc</span>
                                <span id="unread-badge" class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-600 bg-red-100 rounded-full dark:text-red-100 dark:bg-red-600 hidden">
                                    1
                                </span>
                            </a>
                        </li>
            ...
            <script>
                function updateUnreadCount() {
                    fetch('index.php?page=unread_count_api')
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                // 1. Cập nhật Badge chính (cái chuông) - Luôn tồn tại trong DOM
                                const mainBadge = document.getElementById('main-unread-badge');
                                if (mainBadge) {
                                    if (count > 0) {
                                        mainBadge.innerText = formattedCount;
                                ...
                                // 2. Cập nhật Badge trong menu - Chỉ tồn tại khi menu MỞ
                                const dropdownBadge = document.getElementById('unread-badge');
                                if (dropdownBadge) {
                                    dropdownBadge.innerText = formattedCount;
                                    // Trong menu thì không cần ẩn, cứ để số lượng
                                }
                            }
                        })
