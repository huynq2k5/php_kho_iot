<?php
/**
 * $nhom: Đối tượng chứa thông tin nhóm từ Controller (idNhom, maNhom, tenNhom, moTa)
 * $allPermissions: Danh sách tất cả quyền
 * $assignedPermissions: Mảng ID các quyền đã gán
 * $currentMembers: Danh sách nhân sự trong nhóm
 * $availableUsers: Danh sách nhân sự chưa có nhóm
 */
$group = $nhom; 
$isEdit = true;
...
<div class="flex flex-col items-start justify-between w-full gap-4 my-6 sm:flex-row sm:items-center">
    <div>
        <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">
            Sửa nhóm: <span class="text-red-600"><?= htmlspecialchars($group->tenNhom) ?></span>
        </h2>
...
<div class="mb-6 border-b border-gray-200 dark:border-gray-700">
    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="groupTabs">
        <li class="mr-2">
            <button class="inline-block p-4 border-b-2 rounded-t-lg transition-colors duration-150 text-red-600 border-red-600" 
        ...
        <li class="mr-2">
            <button class="inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 transition-colors duration-150" 
                ...
                <i class="fas fa-key mr-2"></i> 2. Phân quyền (<?= count($assignedPermissions ?? []) ?>)
            </button>
        </li>
        <li class="mr-2">
            <button class="inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 transition-colors duration-150" 
                    id="members-tab" type="button" onclick="switchTab('members')">
                <i class="fas fa-users mr-2"></i> 3. Thành viên (<?= count($currentMembers ?? []) ?>)
            </button>
        </li>
    </ul>
</div>

<form action="index.php?page=nhom_xuly_capnhat" method="POST" id="mainForm">
        ...
        <div class="grid gap-6 md:grid-cols-2">
            <div class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                <h4 class="font-semibold text-gray-700 dark:text-gray-300 mb-4 flex items-center">
                    <i class="fas fa-user-check text-green-600 mr-2"></i> Thành viên hiện tại
                </h4>
                <div class="space-y-2 max-h-80 overflow-y-auto pr-2">
                    ...
                    <?php else: ?>
                        ...
                        <div class="flex items-center justify-between p-2 bg-gray-50 rounded-lg dark:bg-gray-700">
                            ...
                            <button type="button" onclick="if(confirm('Xóa khỏi nhóm?')) location.href='index.php?page=nhom_xoa_thanhvien&idNguoiDung=<?= $member->idNguoiDung ?>&idNhom=<?= $groupId ?>'" 
            ...
            <div class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                <h4 class="font-semibold text-gray-700 dark:text-gray-300 mb-4 flex items-center">
                    <i class="fas fa-user-plus text-blue-600 mr-2"></i> Thêm nhân sự vào nhóm
                </h4>
                ...
                <div class="space-y-2 max-h-80 overflow-y-auto pr-2" id="availableUsersList">
                    ...
                    <label class="flex items-center p-2 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 transition-colors">
                        <input type="checkbox" name="selected_users[]" class="sr-only peer" value="<?= $user->idNguoiDung ?>">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400"><?= $user->tenDangNhap ?></p>
                        </div>
                    </label>
                    <?php endforeach; ?>
                </div>
    ...
    <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
        <button type="submit" class="px-5 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 shadow-md">
...
<script>
    function switchTab(tab) {
        ...
        document.getElementById(tab + '-tab').classList.add('text-red-600', 'border-red-600');
    }
